...
fn initialize(
	...
	for poly in polys {
		...
		for vert in &poly[1..] {
			...
			let next = polygon.len() - 1;
			link(last, next, polygon, polygon_range);
			last = next
		}

		link(last, first, polygon, polygon_range);
	}

	if *epsilon < 0.0 {
		*epsilon = bbox.scale() * K_PRECISION;
	}

	result.add_contours(polys);
	// Slightly more than enough, since each hole can cause two extra triangles.
	result.reserve_triangles(polygon.len() + 2 * starts.len());
...
fn clip_if_degenerate(
	...
	if ear_ref.is_short(epsilon)
		|| (ccw(
			...
			ear_ref.right().pos,
			epsilon,
		) == 0 && (ear_ref.left().pos - ear_ref.pos).dot(&(ear_ref.right().pos - ear_ref.pos))
			> 0.0)
	{
		clip_ear(ear, polygon, polygon_range, result);
		let ear_ref = &polygon[ear];
		let left = ear_ref.left().ptr2index(polygon_range);
		let right = ear_ref.right().ptr2index(polygon_range);
		clip_if_degenerate(left, result, polygon, polygon_range, epsilon);
		clip_if_degenerate(right, result, polygon, polygon_range, epsilon);
	}
}
...
fn find_start(
	polygon: &mut Vec<Vert>,
	polygon_range: &Range<usize>,
	hole2bbox: &mut BTreeMap<usize, Box2D>,
	...
	let add_point = |v: usize, polygon: &mut Vec<Vert>| {
		...
		if v.pos.x > max_x && v.is_reflex(epsilon) {
			max_x = v.pos.x;
			start = v.ptr2index(polygon_range);
		}
	};

	if loop_verts(first, polygon, polygon_range, add_point).is_none() {
...
fn triangulate_poly(
	...
	let v = loop_verts(start, polygon, polygon_range, queue_vert);
	if v.is_none() {
		return;
	}
	let mut v = v.unwrap();
	...
	while num_tri > 0 {
		...
		clip_ear(v, polygon, polygon_range, result);
		num_tri -= 1;
...
fn clip_ear(
	...
	link(left_i, right_i, polygon, polygon_range);
...
fn cut_keyhole(
	...
	polygon_range: &Range<usize>,
	outers: &Vec<usize>,
	hole2bbox: &BTreeMap<usize, Box2D>,
	epsilon: f64,
) {
	...
	for &first in outers {
		loop_verts(first, polygon, polygon_range, &mut check_edge);
	}
	...
	let connector = find_closer_bridge(
		start,
		connector.unwrap(),
		polygon,
		polygon_range,
	...
	join_polygons(start, result, polygon, connector, polygon_range, epsilon);
}
...
fn find_closer_bridge(
	...
	for &first in outers {
		loop_verts(first, polygon, polygon_range, &mut check_vert);
	}
...
fn join_polygons(
	...
	link(start, connector, polygon, polygon_range);
	link(new_connector, new_start, polygon, polygon_range);

	clip_if_degenerate(start, result, polygon, polygon_range, epsilon);
	clip_if_degenerate(new_start, result, polygon, polygon_range, epsilon);
	clip_if_degenerate(connector, result, polygon, polygon_range, epsilon);
	clip_if_degenerate(new_connector, result, polygon, polygon_range, epsilon);
}
...
fn link(left: usize, right: usize, polygon: &mut Vec<Vert>, polygon_range: &Range<usize>) {
	...
	polygon[left].right_dir = safe_normalize2(polygon[right].pos - polygon[left].pos);
}
...
impl Vert {
	...
	fn is_short(&self, epsilon: f64) -> bool {
		...
		edge.magnitude_squared() * 4.0 < epsilon * epsilon
	}
	...
	fn ear_cost(&self, epsilon: f64, collider: &IdxCollider, polygon: &Vec<Vert>) -> f64 {
		...
		tree2d::query_2d_tree(&collider.points, ear_box, |point| {
